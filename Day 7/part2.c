...
enum HandType getHandType(Hand *hand){
    ...
    // Hand fakeHand;
    // for(int i = 0)
    // memcpy(&fakeHand, hand, sizeof(Hand));
	...
	for(int i = 0; i < HAND_SIZE; i++){
        ...
        if (duplicateCount == highestDuplicateCount){
            if (cardStrength[card] > cardStrength[mostCommonCard])
                mostCommonCard = card;
        }
		if (duplicateCount > highestDuplicateCount){
    ...
    for(int i = 0; i < HAND_SIZE; i++) {
        Card card = hand->cards[i];
        if (card == 'J') {
            duplicateCounts[i] = highestDuplicateCount + numJokers;
        }
        if (card == mostCommonCard) {
            duplicateCounts[i] += numJokers;
        }
    }


	highestDuplicateCount = duplicateCounts[0];
